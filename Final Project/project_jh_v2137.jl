...
@with_kw struct HAProblem
    ...
    a_max  = 25 # maximum assets DO POPRAWY: ewentualnie np. 2*K
    N_a    = 300 # assets grid 
...
function aiyagari_residual_beta(β, hh, govt, firm, r, w)
    hh = HAProblem(β=β)  # Update β in the household problem
    prices = (r=r, w=w)
...
residual_beta(0.935)
β_star = find_zero(residual_beta, (0.9, 0.96), Bisection(), verbose=true, maxiter=10)
...
###########################################################
######ODTĄD POTENCJALNIE NIEPRZYDATNY KOD
